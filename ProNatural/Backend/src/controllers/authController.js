...
authController.login = async (req, res) => {
  try {
    ...
    let user = await usersModel.findOne({ correo: email });
    let role = "Admin";
    let isCliente = false;
    let isEmpleado = false;

    if (!user) {
      user = await empleadosModel.findOne({ correo: email });
      if (user) {
        role = "Employee";
        isEmpleado = true;
      }
    ...
    if (!user) {
      user = await clientesModel.findOne({ email: email }) || await clientesModel.findOne({ correo: email });
      if (user) {
        role = "Customer";
        isCliente = true;
      }
