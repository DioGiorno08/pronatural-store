  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
        ...
        <span className="text-gray-600 text-xs sm:ml-auto">{filteredReviews.length} resultados</span>
      </div>
      <div className="space-y-4">
        {filteredReviews.length === 0 ? (
          ...
          filteredReviews.map(review => {
            ...
            return (
            <div key={reviewId} className="bg-[#161b22] border border-[#1e2a1e] rounded-xl p-6 hover:border-[#22c55e]/20 transition-all duration-200 group">
              <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-4">
                <div className="flex items-start gap-4 flex-1">
                  <div className="w-10 h-10 bg-[#1b4332] text-[#4ade80] rounded-full flex items-center justify-center text-sm font-bold shrink-0">
                    {(review.name || review.nombreCliente || '?').charAt(0).toUpperCase()}
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-1">
                      <h3 className="text-white text-sm font-bold">{review.name || review.nombreCliente}</h3>
                      <span className="text-gray-600 text-[10px] font-mono">
                        {review.date || (review.createdAt ? new Date(review.createdAt).toLocaleDateString() : 'Reciente')}
                      </span>
                    </div>
                    ...
                    <p className="text-gray-400 text-[13px] leading-relaxed">"{review.comment || review.comentario}"</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 sm:ml-4 shrink-0 self-end sm:self-auto">
                  {confirmDeleteId === reviewId ? (
                    <div className="flex items-center gap-2 animate-[fadeIn_0.2s_ease-out]">
                      ...
                      <button
                        ...
                        className="px-3 py-1.5 bg-gray-800 text-gray-400 text-[10px] font-bold uppercase tracking-wider rounded-lg hover:bg-gray-700 hover:text-white transition-colors cursor-pointer"
                      >
                        No
                      </button>
                    </div>
                    ...
                    <button
                      ...
                      className="opacity-0 group-hover:opacity-100 px-3 py-1.5 bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white text-[10px] font-bold uppercase tracking-wider rounded-lg transition-all cursor-pointer flex items-center gap-2"
                    >
